...
pub struct TitanCrowd {
    ...
    pub slots: u32,
    /// **The radius of the ring of standing places**, in meters — how far from the player a
    /// titan of the crowd comes to rest. Capped per kind at nine tenths of its own
    /// `attack_range_m` (`titan::perception::ring_radius_m`), because a standing place outside
    /// a titan's own reach is a titan that has surrounded you and cannot touch you.
    ///
    /// It does **not** fade. `behaviour.flank_offset_m` has to, because a flank is
    /// perpendicular to the approach and a constant one leaves a titan orbiting forever; the
    /// ring's offset is a rotated copy of the line and always keeps an inward component while
    /// the radius is under the reach. See `titan::perception::ring_radius_m`.
    pub ring_radius_m: f32,
    /// How near his standing place a titan has to be before he counts as **in position**, in
    /// meters. Under it he stops walking and is allowed to swing; over it he is still taking
    /// up his slot and holds his attack (`titan::brain::decide`, and only while `of > 1`).
    ///
    /// It is a real number and not a tolerance of convenience: it is what stops six titans
    /// from all "arriving" at the first point of the ring they touch, which is what the
    /// acceptance sentence of `F-055` fails on.
    pub arrive_m: f32,
}
