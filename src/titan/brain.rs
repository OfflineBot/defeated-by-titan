...
pub(super) fn advance(
    ...
    ) in &mut bodies
    {
        ...
        // **`F-055` reaches the state machine here, and as one bit.** A titan of a crowd takes
        // up his standing place before he swings; a lone titan has no place to take and is
        // always in position, which is what leaves every single-titan measurement in this
        // repository (`F-030`, `F-034`, `q030`, `q031`, `F-032`) untouched to the tick.
        let in_position = slot.of <= 1 || slot.at_slot;
...
pub fn decide(
    ...
    // **`F-055`.** The swing waits for the standing place. `in_position` is true for every
    // titan that is not in a crowd, so this line does nothing at all to a duel — see
    // [`advance`], which is the only place it is ever false.
    let may_swing = cooldown_left == 0 && in_position;
    match state {
        TitanState::Idle => {
            ...
            if tuning.ambush {
                if seen && target.distance_m <= tuning.attack_range_m && may_swing {
                    TitanState::Windup
                } else {
                    TitanState::Idle
                }
        ...
        TitanState::Pursue => {
            if !in_range {
                TitanState::Idle
            } else if target.distance_m <= tuning.attack_range_m && may_swing {
                TitanState::Windup
            } else {
                TitanState::Pursue
            }
...
pub fn aim(
    ...
    to += ring_offset(crowd, slot, tuning.attack_range_m);
...
pub(super) fn walk(
    ...
    ) in &mut bodies
    {
        ...
        // **Where he stops.** Alone that is his own reach, exactly as it was before `F-055`.
        // In a crowd it is his standing place: a titan who stopped at the reach circle would
        // stop at the first point of it he touched, and six titans off one line touch it in
        // one place. `at_slot` is `perception::claim_slots`'s, one tick old — `claim_slots`
        // runs after `advance` on purpose (see `titan/mod.rs`), and one tick of lag on an
        // arrival is 16.7 ms of walking, which is 5 cm.
        let in_position = slot.of <= 1 || slot.at_slot;
        let pursuing = *state == TitanState::Pursue
            && seen
            && (target.distance_m > tuning.attack_range_m || !in_position);
...
mod tests {
    ...
    fn there_is_no_edge_from_pursue_to_strike() {
        ...
        assert_eq!(
            decide(TitanState::Pursue, &clock, 0, true, true, &timing(), &tuning(), &at(1.0)),
            TitanState::Windup
        );
    }
    ...
    fn a_cooldown_holds_the_titan_in_pursue_even_inside_reach() {
        let clock = StateClock::default();
        assert_eq!(
            decide(TitanState::Pursue, &clock, 7, true, true, &timing(), &tuning(), &at(1.0)),
            TitanState::Pursue
        );
    }
    ...
    fn losing_the_target_falls_back_to_idle() {
        ...
        assert_eq!(
            decide(TitanState::Pursue, &clock, 0, false, true, &timing(), &tuning(), &nobody),
            TitanState::Idle
        );
        ...
        assert_eq!(
            decide(TitanState::Idle, &clock, 0, false, true, &timing(), &tuning(), &at(99.0)),
            TitanState::Idle
        );
    }
    ...
    fn the_attack_runs_windup_strike_recover_and_back_to_pursue() {
        ...
        for (state, ticks_in_state, wanted) in [
            ...
            assert_eq!(
                decide(state, &clock, 0, true, true, &t, &u, &at(1.0)),
                wanted,
    ...
    fn death_is_a_one_way_street() {
        ...
        assert_eq!(
            decide(TitanState::Death, &clock, 0, true, true, &timing(), &tuning(), &at(1.0)),
            TitanState::Death
        );
    }
    ...
    fn every_timed_state_reports_the_length_it_is_ended_on() {
        ...
        for state in [TitanState::Windup, TitanState::Strike, TitanState::Recover] {
            ...
            assert_eq!(
                decide(state, &last_inside, 0, true, true, &t, &u, &at(1.0)),
                state,
                "{state:?} ended one tick before its own `state_ticks`"
            );
            assert_ne!(
                decide(state, &first_after, 0, true, true, &t, &u, &at(1.0)),
                state,
    ...
    /// goes red.
    ///
    /// **`Idle` hands over to `Pursue` and never straight to `Windup`, at any distance.** That
    /// edge belongs to the ambusher and to nobody else (`F-061`): a lurker's whole identity is
    /// that he has no `Pursue` and goes from crouching to swinging in one decision. Give it to
    /// the husk as well and the lurker is a slow husk. So the close case is checked one edge
    /// further down instead — `Pursue` at one metre IS the swing, on the next tick.
    #[test]
    fn f051_a_titan_that_has_not_noticed_you_does_not_come_however_close_you_are() {
        let clock = StateClock::default();
        assert_eq!(
            decide(TitanState::Idle, &clock, 0, false, true, &timing(), &tuning(), &at(1.0)),
            TitanState::Idle,
            "one metre away and unnoticed"
        );
        assert_eq!(
            decide(TitanState::Idle, &clock, 0, true, true, &timing(), &tuning(), &at(1.0)),
            TitanState::Pursue,
            "one metre away and noticed"
        );
        // And the far end: noticed at 99 m is a walk, not a stand.
        assert_eq!(
            decide(TitanState::Idle, &clock, 0, true, true, &timing(), &tuning(), &at(99.0)),
            TitanState::Pursue
        );
        // The two `Pursue`s above are the same word for two different things, so the close one
        // is followed one edge further: at one metre the swing is the very next decision, and
        // at 99 m it is not. Without this pair the middle assert could be satisfied by a
        // `decide` that had never heard of `attack_range_m`.
        assert_eq!(
            decide(TitanState::Pursue, &clock, 0, true, true, &timing(), &tuning(), &at(1.0)),
            TitanState::Windup,
            "one metre away, noticed, and already coming"
        );
        assert_eq!(
            decide(TitanState::Pursue, &clock, 0, true, true, &timing(), &tuning(), &at(99.0)),
            TitanState::Pursue
        );
        // And the gate holds on the way back out: awareness lost is a titan that stops.
        assert_eq!(
            decide(TitanState::Pursue, &clock, 0, false, true, &timing(), &tuning(), &at(1.0)),
            TitanState::Idle,
            "one metre away and forgotten again"
        );
    }
}
