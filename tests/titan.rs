...
/// put on one line 60 m from a player who does not move, and walked for 900 ticks. Then the
...
fn f055_six_titans_on_one_player_stand_in_six_places() {
    fn run(ring_radius_m: Option<f32>) -> (f32, Vec<u32>) {
        ...
        // On a line 60 m out, 2 m apart — inside `aggro_radius_m` 45? No: 60 m is outside it,
        // which is the point. They are walked in by the ring, not by the spawn.
        for i in 0..6 {
            spawn(&mut app, "husk", Vec3::new(i as f32 * 2.0 - 5.0, 0.0, -35.0));
        }
        ticks(&mut app, 900);
