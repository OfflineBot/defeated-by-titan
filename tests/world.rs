...
use defeated_by_titan::world::anchor::{
    generate_points, AnchorField, AnchorKind, Hemisphere, CORNER_INSET_M, MIN_SPACING_M,
};
use defeated_by_titan::shared::ModelAnchors;
...
// ---------------------------------------------------------------------------------------
// `F-021` / `F-022` / `F-023` / `F-031a` — the discrete anchor points.
//
// `F-003` tagged every box in the district anchorable and that is level ONE: *which surfaces
// hold a rope*. It is a boolean per cuboid and it cannot say **where** on a 700 m wall the
// rope goes. These tests measure level two — the list of individually addressable points with
// metadata that `F-023`'s candidate search, `F-026`'s markers and `F-027`'s density cap all
// read.
//
// ⚠️ The trap this file has walked into six times (`FIND-103`) is a test that asks the
// function and the screen the same question. So the assertions here are against **`maps.ron`
// and plain geometry**, never against `AnchorField`'s own opinion: a rung is at a height the
// FILE puts a wall band at, a corner is inside a footprint the FILE gives, and the hemisphere
// split is checked against the dot product recomputed in the test.
// ---------------------------------------------------------------------------------------

fn a_block(name: &str, center: Vec3, size: Vec3, anchorable: bool) -> BlockPlan {
    BlockPlan {
        name: name.to_string(),
        center_m: center,
        size_m: size,
...
/// The real district's field, out of the real plan.
fn ashgate_field() -> AnchorField {
    let d = data();
    let map = d.current_map().expect("maps.ron: current must exist");
    AnchorField::from_plan(&plan_blocks(&d, map), map.size_m)
}

#[test]
fn f021_the_district_has_a_complete_point_list_and_every_point_is_addressable() {
    let field = ashgate_field();

    assert!(
        field.len() > 0,
        "F-021: `maps.ron: current` has no anchor points at all — the whole district is \
         tagged anchorable and not one place in it is a place you can aim at"
    );

    // *Einzeln inspizierbar*: every point answers to its own address, and the block it names
    // is a block that was really planned.
    for (i, point) in field.points().iter().enumerate() {
        let by_address = field.get(i as u32).unwrap_or_else(|| {
            panic!("F-021: point {i} exists in the list but `get({i})` does not find it")
        });
        assert_eq!(by_address, point, "F-021: point {i} is not the point at address {i}");
        let name = field.block_name(point).unwrap_or_else(|| {
            panic!(
                "F-021: point {i} at {:?} names block {} — and there is no such block",
                point.position_m, point.block
            )
        });
        assert!(!name.is_empty(), "F-021: point {i} belongs to a block with no name");
        assert!(
            point.quality > 0.0 && point.quality <= 1.0,
            "F-021: point {i} ({name}) has a quality of {} — the Guete-Basiswert is 0..1",
            point.quality
        );
    }

    // Determinism, which is the only way this survives multiplayer (`docs/multiplayer.md`).
    let again = ashgate_field();
    assert_eq!(
        field.len(),
        again.len(),
        "F-021: two builds of the same map produced different point counts"
    );
    assert_eq!(field.points(), again.points(), "F-021: the point list is not deterministic");
}

#[test]
fn f022_a_roof_gets_corners_and_a_floor_plate_gets_nothing() {
    // A house out of the district's own height band.
    let house = a_block("house_1_0", Vec3::new(20.0, 5.5, -30.0), Vec3::new(12.0, 11.0, 10.0), true);
    let points = generate_points(&house, 7);

    let corners: Vec<_> = points.iter().filter(|p| p.kind == AnchorKind::Corner).collect();
    assert_eq!(
        corners.len(),
        4,
        "F-022: a cuboid roof has four corners, this one produced {} — {:?}",
        corners.len(),
        corners.iter().map(|c| c.position_m).collect::<Vec<_>>()
    );

    let roof_y = house.center_m.y + house.size_m.y * 0.5;
    for c in &corners {
        assert!(
            (c.position_m.y - roof_y).abs() < 1e-3,
            "F-022: a roof corner sits at y = {} and the roof of this box is at {roof_y}",
            c.position_m.y
        );
        assert_eq!(c.normal, Vec3::Y, "F-022: a roof point faces up");
        assert_eq!(c.block, 7, "F-022: a point must carry the index it was generated for");
        assert_eq!(c.name, None, "F-022: a generated point has no model name");
        // Inside the footprint, by exactly the inset — not on the edge and not off it.
        let d = (c.position_m - house.center_m).abs();
        assert!(
            (d.x - (house.size_m.x * 0.5 - CORNER_INSET_M)).abs() < 1e-3
                && (d.z - (house.size_m.z * 0.5 - CORNER_INSET_M)).abs() < 1e-3,
            "F-022: corner {:?} is not inset {CORNER_INSET_M} m from the box at {:?} / {:?}",
            c.position_m,
            house.center_m,
            house.size_m
        );
    ...
    // The ground slab of Ashgate — 700 x 0.2 x 700 m. Without the thickness rule this one
    // block alone would carry more points than the whole rest of the district.
    let slab = a_block("block_0", Vec3::new(0.0, -0.1, 0.0), Vec3::new(700.0, 0.2, 700.0), true);
    assert!(
        generate_points(&slab, 0).is_empty(),
        "F-022: the 0.2 m ground slab produced {} anchor points — a floor is not a feature",
        generate_points(&slab, 0).len()
    );

    // And a block the file took the tag away from stays untagged (`F-003`'s exception list).
    let untagged =
        a_block("block_9", Vec3::new(0.0, 5.0, 0.0), Vec3::new(10.0, 10.0, 10.0), false);
    assert!(
        generate_points(&untagged, 9).is_empty(),
        "F-022: an unanchorable block must not contribute anchor points"
    );
}

#[test]
fn f022_no_block_puts_two_points_closer_than_the_minimum_spacing() {
    let field = ashgate_field();
    // Per block, because that is where `F-022`'s Mindestabstand is applied; across blocks it
    // is `F-031a`'s cluster count.
    let mut by_block: BTreeMap<u32, Vec<Vec3>> = BTreeMap::new();
    for p in field.points() {
        by_block.entry(p.block).or_default().push(p.position_m);
    }
    let mut worst = f32::INFINITY;
    let mut worst_block = 0u32;
    for (block, points) in &by_block {
    ...
    assert!(
        worst >= MIN_SPACING_M - 1e-3,
        "F-022: block {} ({}) has two points {worst:.3} m apart, the minimum is {MIN_SPACING_M}",
        worst_block,
        field.blocks().get(worst_block as usize).map(String::as_str).unwrap_or("?")
    );
}

#[test]
fn f022_the_wall_stacks_into_a_ladder_of_rungs_every_fifteen_metres() {
    let d = data();
    let map = d.current_map().expect("maps.ron: current must exist");
    let plan = plan_blocks(&d, map);
    let field = AnchorField::from_plan(&plan, map.size_m);

    // The north wall of Ashgate stands at z = -120 and `maps.ron` builds it out of 15 m
    // courses from y = 0 to y = 120. Its rungs are therefore the tops of those courses, and
    // that is the `hook.gesims_15..105` ladder of the pack (`FIND-134`) without one collider
    // being re-cut.
    //
    // ⚠️ The heights come from the FILE, not from the field: these are the eight band tops
    // `maps.ron` places at (0, 7.5, -120), (0, 22.5, -120) ... (0, 112.5, -120).
    let wall_tops: Vec<f32> = plan
        .iter()
        .filter(|b| {
            (b.center_m.z - (-120.0)).abs() < 25.0
                && b.center_m.x.abs() < 5.0
                && b.size_m.x > 200.0
                && b.size_m.y >= 10.0
        })
    ...
    assert!(
        wall_tops.len() >= 6,
        "the fixture moved: maps.ron no longer has a stack of tall wall courses at z = -120, \
         found {wall_tops:?}"
    );

    let near_wall = field.near(Vec3::new(0.0, 60.0, -120.0), 120.0);
    for top in &wall_tops {
        let hit = near_wall.iter().filter_map(|i| field.get(*i)).any(|p| {
            (p.position_m.y - top).abs() < 1.0 && p.position_m.x.abs() < 60.0
        });
        assert!(
            hit,
            "F-022: the wall course whose top is at y = {top} carries no anchor point within \
             60 m of the gate — the ladder has a missing rung, and that is the one place in \
             this map a player is guaranteed to be climbing"
        );
    }

    let mut rungs: Vec<i32> = near_wall
        .iter()
        .filter_map(|i| field.get(*i))
        .filter(|p| p.position_m.x.abs() < 60.0)
        .map(|p| p.position_m.y.round() as i32)
        .collect();
    rungs.sort_unstable();
    rungs.dedup();
    assert!(
        rungs.len() >= 6,
        "F-022: the wall in front of the gate offers only {} distinct heights: {rungs:?}",
        rungs.len()
    );
}

#[test]
fn f023_q_never_delivers_a_point_right_of_the_camera_axis() {
    let field = ashgate_field();

    // Standing on the main street, looking north at the wall.
    let eye = Vec3::new(0.0, 12.0, -20.0);
    let forward = Vec3::new(0.0, 0.2, -1.0).normalize();
    let range_m = 200.0;
    let found = field.candidates(eye, forward, Vec3::Y, range_m, 130.0, 90.0);

    assert!(
        !found.is_empty(),
        "F-023: standing at {eye:?} looking at a 120 m wall, the candidate search found nothing"
    );

    // ⚠️ The side is re-derived here from the dot product, NOT read back off the candidate —
    // otherwise this asks the function the same question it asked itself (`FIND-103`).
    let right_axis = forward.cross(Vec3::Y).normalize();
    for c in &found {
        let p = field.get(c.index).expect("a candidate must address a real point");
        let lateral = (p.position_m - eye).dot(right_axis);
        let expected = if lateral < 0.0 { Hemisphere::Left } else { Hemisphere::Right };
        assert_eq!(
            c.side, expected,
            "F-023: point {} at {:?} has a lateral component of {lateral} and was sorted {:?}",
            c.index, p.position_m, c.side
        );
        assert!(
            c.distance_m <= range_m + 1e-3,
            "F-023: candidate {} is {} m away and the reach is {range_m}",
            c.index,
            c.distance_m
        );
        assert!(
            (p.position_m - eye).dot(forward) > 0.0,
            "F-023: candidate {} stands behind the camera",
            c.index
        );
    }

    let left = field.best_of(&found, Hemisphere::Left, 12);
    let right = field.best_of(&found, Hemisphere::Right, 12);
    assert!(!left.is_empty() && !right.is_empty(), "F-023: one hemisphere came back empty");
    assert!(
        left.len() <= 12 && right.len() <= 12,
        "F-027: the cap of twelve markers per side is not held: {} / {}",
        left.len(),
        right.len()
    );
    assert!(
        left.iter().all(|c| c.side == Hemisphere::Left)
            && right.iter().all(|c| c.side == Hemisphere::Right),
        "F-023: Q's list is not purely left, or E's is not purely right"
    );
    // Best first — `F-027`'s Bewertungsfunktion has to be an order, not a shuffle.
    for w in left.windows(2) {
        assert!(w[0].score >= w[1].score, "F-027: the left list is not sorted best first");
    }

    // The control (rule 5): a cone of nothing must find nothing. If this still returns
    // points, the cone is not being applied and the test above measured only the range.
    let blind = field.candidates(eye, forward, Vec3::Y, range_m, 0.0, 0.0);
    assert!(
        blind.is_empty(),
        "F-023: a cone of 0 x 0 degrees still returned {} candidates — the cone is not cutting",
        blind.len()
    );
    let behind = field.candidates(eye, -forward, Vec3::Y, range_m, 20.0, 20.0);
    assert!(
        behind.iter().all(|c| c.index != found[0].index),
        "F-023: turning around still returns the best point in front"
    );
}

#[test]
fn f031a_the_shipped_district_passes_its_own_validation_report() {
    let field = ashgate_field();
    let report = field.validate();
    assert!(
        report.is_clean(),
        "F-031a: `maps.ron: current` does not pass validation — {report:?}. No map ships \
         without a clean report."
    );
    assert!(
        report.points > 1000,
        "F-031a: a 700 x 700 m district with 2871 blocks reports only {} anchor points",
        report.points
    );
}

#[test]
fn f021_a_models_named_hook_point_lands_in_the_field_in_world_space() {
    let plan = vec![a_block("haus", Vec3::new(10.0, 5.0, 0.0), Vec3::new(10.0, 10.0, 10.0), true)];
    let mut field = AnchorField::from_plan(&plan, (700.0, 700.0));
    let before = field.len();

    // What the pack really ships: an eaves and a ridge, in the model root's own frame.
    let mut anchors = ModelAnchors::default();
    anchors.0.insert("hook.traufe".to_string(), Vec3::new(0.0, 8.0, 4.0));
    anchors.0.insert("hook.first".to_string(), Vec3::new(0.0, 10.0, 0.0));
    anchors.0.insert("cortex".to_string(), Vec3::new(0.0, 9.0, 0.0));
    let to_world = GlobalTransform::from(Transform::from_translation(Vec3::new(10.0, 0.0, 0.0)));

    field.adopt_named(0, &to_world, &anchors);
    assert_eq!(
        field.len() - before,
        2,
        "F-021: two `hook.*` empties and one `cortex` must add exactly two anchor points"
    );

    let traufe = field
        .points()
        .iter()
        .find(|p| p.name.as_deref() == Some("traufe"))
        .expect("F-021: `hook.traufe` did not reach the field");
    assert_eq!(
        traufe.position_m,
        Vec3::new(10.0, 8.0, 4.0),
        "F-021: a named point must land in WORLD space — the model root stands at x = 10"
    );
    assert_eq!(traufe.kind, AnchorKind::Named);
    assert_eq!(traufe.quality, AnchorKind::Named.quality());
    assert!(!traufe.dynamic, "F-021: a house does not move");

    // Idempotent: `Changed<ModelAnchors>` can fire twice for one entity and the field may not
    // grow a second copy.
    let after_once = field.len();
    field.adopt_named(0, &to_world, &anchors);
    assert_eq!(
        field.len(),
        after_once,
        "F-021: adopting the same model's anchors twice doubled its points"
    );

    // And the named point is findable through the same query everything else uses.
    let near = field.near(Vec3::new(10.0, 8.0, 4.0), 1.0);
    assert!(
        near.iter().filter_map(|i| field.get(*i)).any(|p| p.name.as_deref() == Some("traufe")),
        "F-021: a named point is in the list but not in the grid — `near` cannot see it"
    );
}

#[test]
fn f031a_a_point_inside_another_block_is_unreachable_and_never_reaches_the_field() {
    // A small shed standing entirely inside a big shed. Its roof is a roof only on paper: a
    // rope that reaches it goes through the outer wall. `F-031a` calls the class
    // *unerreichbare Punkte*, and Ashgate really had 416 of them (2026-08-25) before the
    // builder started dropping them.
    let plan = vec![
        a_block("outer", Vec3::new(0.0, 20.0, 0.0), Vec3::new(40.0, 40.0, 40.0), true),
        a_block("inner", Vec3::new(0.0, 5.0, 0.0), Vec3::new(8.0, 10.0, 8.0), true),
    ];
    let field = AnchorField::from_plan(&plan, (700.0, 700.0));

    assert!(
        field.points().iter().all(|p| p.block != 1),
        "F-031a: the swallowed block contributed {} points — every one of them is a marker on \
         a surface the rope cannot get to",
        field.points().iter().filter(|p| p.block == 1).count()
    );
    assert!(
        field.points().iter().any(|p| p.block == 0),
        "the control: the OUTER block must still carry its own roof points, or this test \
         would pass with a builder that emits nothing at all"
    );
    assert_eq!(
        field.validate().buried,
        0,
        "F-031a: the report still finds buried points after the builder dropped them"
    );

    // And the report has to be able to SEE one — otherwise `buried == 0` above is a number
    // that cannot move, and a check that cannot fail is not a check (`FIND-103`).
    let mut with_a_bad_one = field.clone();
    let mut anchors = ModelAnchors::default();
    anchors.0.insert("hook.inside".to_string(), Vec3::new(0.0, 5.0, 0.0));
    with_a_bad_one.adopt_named(1, &GlobalTransform::IDENTITY, &anchors);
    assert_eq!(
        with_a_bad_one.validate().buried,
        1,
