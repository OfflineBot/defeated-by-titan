# FINDINGS — archive, the entries up to `FIND-184`

Updated: 2026-08-29 · Stage: 🟧 (measured, and moved here to keep the live file readable)

> **Why this file exists.** `docs/FINDINGS.md` had grown to **812 kB / 209 entries** — while
> `CLAUDE.md`'s own rule about it still said *108 kB, ~27 000 tokens*, i.e. it was stale by a
> factor of **7.5**. Every agent is told to grep that file rather than open it, and at 812 kB
> even grepping it well is a skill. **Nothing here is retired or wrong**: it is the record up to
> `FIND-184`, and the live file keeps `FIND-185` onward.
>
> **Read it the same way:** the index below, then `sed -n 'a,bp'` for the one entry you want.
> Never open it whole.
