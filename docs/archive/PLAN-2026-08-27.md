# PLAN — archive, the 2026-08-27 survey's questions and raw reader maps

Updated: 2026-09-01 · Stage: 🟧 (a snapshot, kept whole)

> **Why this file exists.** `docs/PLAN.md` §5 listed the 23 questions as they stood on
> 2026-08-27 and §6 held the six readers' raw maps. **All 23 were put to him and answered** —
> the live record is `docs/QUESTIONS.md` and its archive — and the reader maps describe a tree
> that has since changed under them (the anchor field is gone, the terrain is a height field,
> the titans are twice the size, the wall is cut into modules). Kept because the *method* is
> worth reading and because the maps say what was true when the plan was written.
