# BUGS — archive, the ones that are closed

Updated: 2026-08-29 · Stage: 🟧 (each carries its repro, its fix and the control that moved)

> **Why this file exists.** `docs/BUGS.md` was **125 kB across 27 headings** — about 6 kB per bug
> — and mixed closed with open, so it could not answer *what is broken right now*. Everything
> here is fixed, superseded or won't-fix, **with the repro that found it kept**, because a repro
> is the thing you want back when the same shape returns.
>
