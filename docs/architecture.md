...
| ⚠️ `Gas` — **SECOND WRITER, 2026-08-12, and this is a known violation, not a design** | `mission::hub::refuel_at_stations` calls `Gas::refill` in `PostStep`, hub phase only, at a station. `vector::gas` still owns every debit. The two are disjoint **by phase**, which is why it works today — and "disjoint by phase" is exactly the kind of reasoning rule 4 exists to refuse, because over the network it becomes two authorities on one field | **Queued fix, `docs/NEXT.md`:** the hub sends a `RefuelRequest` message and `vector::gas` applies it, restoring one writer. ~30 lines. Reported by the agent that wrote it rather than hidden — `FINDINGS.md` FIND-057 §2 carries the two alternatives and the rollback |
